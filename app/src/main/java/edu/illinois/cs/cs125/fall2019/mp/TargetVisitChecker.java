...
public class TargetVisitChecker {
    ...
    public static int getTargetWithinRange(final double[] latitudes, final double[] longitudes, final int[] path,
        ...
        /*
            ...
            if distance to target is within range && it has not been visited (is empty)
                   return target number;

            the path array does not necessarily have the targets in order so somehow check which ones
            are captured/still need to be captured...maybe copy path and order it correclty?
         */
        int target = -1;
        boolean isInPath = false;
        if (path.length > 0) {
            for (int i = 0; i < path.length; i++) {
                double curRange = LatLngUtils.distance(currentLatitude, currentLongitude, latitudes[i], longitudes[i]);
                if (curRange <= range) {
                    for (int index : path) {
                        if (i == index) {
                            isInPath = true;
    ...
    public static boolean checkSnakeRule(final double[] latitudes, final double[] longitudes, final int[] path,
        ...
        int numVisited = 0;
        //check how many targets have been visited
        int lastInd = -1;
        ...
        if (numVisited > 1) {
            for (int i = 0; i < path.length - 1; i++) {
                // make sure to traverse in the same order as path
                int a = path[i];
                int b = path[i + 1];
                // if the index is an unvisited target then change to tryVisit index
                if (b == -1) {
                    b = tryVisit;
                }
                if (a != -1) {
                    LatLng preTryPos = new LatLng(latitudes[path[numVisited - 1]], longitudes[path[numVisited - 1]]);
                    LatLng tryPos = new LatLng(latitudes[tryVisit], longitudes[tryVisit]);
                    LatLng aPos = new LatLng(latitudes[a], longitudes[a]);
                    LatLng bPos = new LatLng(latitudes[b], longitudes[b]);
                    boolean doCross = LineCrossDetector.linesCross(preTryPos, tryPos, aPos, bPos);
                    if (doCross) {
                        return false;
                    }
                }
