...
public final class GameActivity extends AppCompatActivity {
    ...
    /** AreaDivider object used for Area mode of the game. */
    private AreaDivider areaDivider;

    /** Tracks the cells that have been visited during Area mode gameplay. */
    private boolean[][] cellsVisited;

    /** Area Mode: Most recent visited cell's x coordinate. */
    private int mostRecentX;

    /** Area Mode: Most recent visited cell's y coordinate. */
    private int mostRecentY;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        mostRecentX = Integer.MIN_VALUE;
        mostRecentY = Integer.MIN_VALUE;
        Log.i(TAG, "Creating");
        ...
        if (mode.equals("target")) {
            ...
            proxThreshold = getIntent().getIntExtra("proxThreshold", PROXIMITY_THRESHOLD);
        }
        if (mode.equals("area")) {
            double north = getIntent().getDoubleExtra("areaNorth", 0.0);
            double south = getIntent().getDoubleExtra("areaSouth", 0.0);
            double east = getIntent().getDoubleExtra("areaEast", 0.0);
            double west = getIntent().getDoubleExtra("areaWest", 0.0);
    ...
    public void updateLocation(final double latitude, final double longitude) {
        ...
        if (mode.equals("target")) {
            int target  = TargetVisitChecker.getTargetWithinRange(targetLats, targetLngs, path,
                    latitude, longitude, getIntent().getIntExtra("proximityThreshold", PROXIMITY_THRESHOLD));
            //visit the target
            if (target > 0) {
                boolean capturable = TargetVisitChecker.checkSnakeRule(targetLats, targetLngs, path, target);
                if (capturable) {
                    int targIndex = TargetVisitChecker.visitTarget(path, target);
                    changeMarkerColor(targetLats[target], targetLngs[target], CAPTURED_MARKER_HUE);
                    if (targIndex > 0) {
                        LatLng start = new LatLng(targetLats[path[targIndex - 1]], targetLngs[path[targIndex - 1]]);
                        LatLng end = new LatLng(targetLats[path[targIndex]], targetLngs[path[targIndex]]);
        ...
        if (mode.equals("area")) {
            //initially any cell can be captured
            //captures are only possible if the cell is un-captured && shares one side with most
            //  recent captured cell
            // If captured add a polygon to Google maps control
            int capturedCells = 0;
            LatLng currentLocation = new LatLng(latitude, longitude);
            int x = areaDivider.getXCoordinate(currentLocation);
            int y = areaDivider.getYCoordinate(currentLocation);
            LatLngBounds bounds = areaDivider.getCellBounds(x, y);
            LatLng ne = bounds.northeast;
            LatLng sw = bounds.southwest;
            LatLng nw = new LatLng(ne.latitude, sw.longitude);
            LatLng se = new LatLng(sw.latitude, ne.longitude);

            //count how many captured
            ...
            if (bounds.contains(new LatLng(latitude, longitude))) {
                //make sure coordinate value is valid and not equal to the most recently visited
                if (x < areaDivider.getXCells() && y < areaDivider.getYCells() && !(mostRecentY == y
                        && mostRecentX == x)) {
                    boolean draw = false;

                    //if none captured then capture the current and draw
                    if (capturedCells == 0) {
                        cellsVisited[x][y] = true;
                        draw = true;
                    } else {
                        // if current un-captured
                        if (!cellsVisited[x][y]) {
                            boolean doesShareSide = false;
                            //if on same y value and left or right of current
                            if (y == mostRecentY && (x - 1 == mostRecentX || x + 1 == mostRecentX)) {
                                doesShareSide = true;
                            //if on same x value and above or below current
                            } else if (x == mostRecentX && (y - 1 == mostRecentY || y + 1 == mostRecentY)) {
                                doesShareSide = true;
                            }
                            //if shared side then capture and draw
                            if (doesShareSide) {
                                cellsVisited[x][y] = true;
                                draw = true;
                            }
                    ...
                    //actually draw and set the mostRecent_ to the current coordinates
                    if (draw) {
                        PolygonOptions cell = new PolygonOptions();
                        cell.add(nw, ne, se, sw);
                        cell.fillColor(PLAYER_COLOR);
                        map.addPolygon(cell);
                        mostRecentX = x;
                        mostRecentY = y;
                    }
     ...
     * @param end the position of other endpoint of the line.
